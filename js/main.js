...
const menuLinks = document.querySelectorAll('.menu__link'); // все ссылки меню
...
// Плавный переход и закрытие меню при клике на ссылку
...
document.addEventListener('DOMContentLoaded', () => {
  // выбираем все кнопки корзины (и на ПК, и на телефоне)
  const cartButtons = document.querySelectorAll('[data-cart-btn], #cart-btn');
  ...
  let cart = []; // { key, title, price, quantity }

  // ---- Универсальное открытие корзины ----
  cartButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      cartPopup.classList.add('active');
      cartPopup.setAttribute('aria-hidden', 'false');
      document.body.style.overflow = 'hidden'; // блокируем прокрутку под корзиной
    });
  });
  ...
  orderBtn.addEventListener('click', (e) => {
    ...
    // пример очистки после оформления
    // cart = [];
    // renderCart();
    // cartPopup.classList.remove('active');
  });
